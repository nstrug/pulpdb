...
def main():
    ...
    for repo in my.repos.listEnabled():
        repoid = myrepoids.popleft()
        #print("repoid = %s" % repoid)
        #sys.exit()
        # create repo in db if needed
        # print repo.__dict__
        #sys.exit()
        #print repo.name
        #print repo.urls
        #sys.exit()
        cur.execute("""SELECT 1 FROM repos WHERE name = %s""", (repo.name,))
        repo_id = cur.fetchone()
        if not repo_id:
            print("repo %s does not exist in db, inserting" % repo.name)
            cur.execute("""INSERT INTO repos (name, repoid) VALUES (%s, %s)""", (repo.name,repoid,))
            conn.commit()
        else:
            print("repo %s already exists in db, skipping" % repo.name)
        cur.execute("""SELECT repo_id FROM repos WHERE name = %s""", (repo.name,))
        ...
        for pkg in download_list:
            ...
            pkg.repo.cache = 0
            # create rpm in db if needed
            bigname = pkg.name + "-" + pkg.version + "-" + pkg.release + "." + pkg.arch
            cur.execute("""SELECT 1 FROM rpms WHERE bigname = %s""", (bigname,))
            if not cur.fetchone():
                print("%s does not exist in db, inserting" % bigname)
                cur.execute("""INSERT INTO rpms (name, bigname, ver, rel, arch, path) VALUES (%s, %s, %s, %s, %s, %s)""", (pkg.name, bigname, pkg.version, pkg.release, pkg.arch, pkg.localpath,))
                cur.execute("""SELECT rpm_id FROM rpms WHERE bigname = %s""", (bigname,))
                rpm_id = cur.fetchone()[0]
                cur.execute("""INSERT INTO rpms_repos (rpm_id, repo_id) VALUES (%s, %s)""", (rpm_id, repo_id))
            else:
                print("%s exists in db, skipping" % bigname)
